...
export class ChatGateway {
  ...
  initialize(io: Server) {
    io.on('connection', (client: Socket) => {
      ...
      // Handle callSid registration from client
      client.on('registerCall', (data: { callSid: string }) => {
        ...
        this.logger.log(`CallSid ${callSid} registered with socket ${client.id}`);
      ...
      client.on('unregisterCall', (data: { callSid: string }) => {
        ...
        this.logger.log(`CallSid ${callSid} unregistered from socket ${client.id}`);
      });

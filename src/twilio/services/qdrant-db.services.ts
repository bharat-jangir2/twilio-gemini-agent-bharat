...
IMPORTANT: This question is not in your predefined knowledge base. You should respond naturally saying that you don't have specific information about this topic in your knowledge base, but you're happy to help with other questions that are available. Do NOT use your general knowledge to answer this question. Provide your response in a natural, conversational way suitable for a phone conversation.`;
  ...
  async addToExistingCollection(collectionName: string, document: QdrantDocument): Promise<void> {
    try {
      ...
      this.logger.log(`Adding document to collection: ${collectionName}`);
      this.logger.log(`Document ID: ${document.id}`);
      this.logger.log(`Embedding length: ${document.embedding.length}`);
      this.logger.log(`Content: ${document.content.substring(0, 100)}...`);
      ...
      this.logger.log(`Generated point ID: ${pointId}`);
      ...
      if (!response.ok) {
        ...
        this.logger.error(`Failed to add document to collection: ${response.statusText}, Response: ${errorText}`);
      ...
      this.logger.log(`Added document ${document.id} to collection ${collectionName}`);
    } catch (error) {
  ...
  private generatePointId(stringId: string): number | string {
    ...
    // Otherwise, generate a hash-based numeric ID
    let hash = 0;

  ...
  async getAnswerUsingChromaRAG(
    ...
    try {
      ...
      if (relevantDocs && relevantDocs.length > 0) {
        ...
        this.conversationLogger.updateSourceQdrantHighConfidence(interaction, relevantDocs.map(doc => ({
          content: doc,
        ...
        responseText =
          typeof response.content === 'string'
            ? 'Context not found in vector database. ' + response.content
            : 'Context not found in vector database. ' + JSON.stringify(response.content);
